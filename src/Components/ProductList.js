...
function ProductList({ productos, agregarProductoSeleccionado, searchTerm }) {
  ...
  return (
    <div className="productos-grid" style={{ overflowY: "auto", borderRadius: '10px' }}>
      {filteredProducts.map((producto) => (
        <div key={producto.id} className="producto-card">
          <img
            src={producto.image || defaultProductImage}
            alt={producto.name}
            style={{ height: "auto", borderRadius: '20px', border: '1px solid' }}
          />
      ...
      <Dialog visible={modalShow} closable={false} style={{ width: '30vw', background: 'white', border: '1px solid', borderRadius: '10px' }} className="card-dialog" >
        <div className="close-button-containerModal">
          <button className="close-buttonModal" onClick={closeModal}>
            X
          </button>
        </div>
        <div className="card-content" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <img
            ...
            style={{ width: "60%", height: "auto", borderRadius: '20px', border: '1px solid' }}
          />
          <h3 style={{ textAlign: 'center', marginTop: '15px' }}>{seleccionarProducto?.name}</h3>
          <h4 >Descripción</h4>
          <p>{seleccionarProducto?.description}</p>
        </div>
        <div className="card-content" style={{ display: 'flex', flexDirection: 'column', marginLeft: '30px' }}>
        ...
        <div className="edit-button-container" style={{ marginBottom: '10px' }}>
          <Button className="edit-button" label="Editar" icon="pi pi-pencil" onClick={() => openModal2()} />
        </div>
      </Dialog>
      <Dialog visible={modal2} onHide={closeModal2} style={{ width: '50vw', background: 'white' }} breakpoints={{ '960px': '75vw', '641px': '100vw' }}>
        <h3 style={{ textAlign: 'center' }}>Actualizar información del producto</h3>
        <UpdateProduct
